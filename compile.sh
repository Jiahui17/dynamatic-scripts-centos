...
cp $COMP_DIR/$KERNEL_NAME.dot $OUTPUT_DIR/${KERNEL_NAME}_optimized.dot

$DOT2VHDL_BIN $OUTPUT_DIR/${KERNEL_NAME}_optimized \
...
mkdir -p $hdl_path
mv "${bench_path}/reports/${KERNEL_NAME}_optimized.vhd" "$hdl_path/${KERNEL_NAME}_optimized.vhd"
...
cp "${bench_path}"/*.{c,h} "${verify_dir}"/C_SRC
cp ../integration_utils.h $SRC_DIR/sim
...
$HLS_VERIFIER_BIN cover -aw32 "../C_SRC/$KERNEL_NAME.c" "../C_SRC/$KERNEL_NAME.c" "$KERNEL_NAME"
